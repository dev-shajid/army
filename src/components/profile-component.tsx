...
export default function ProfileComponent() {
    ...
    const name = metadata.name || user.email?.split('@')[0] || 'Unknown User'
    const initials = name
        .split(' ')
        .map((n: string) => n[0])
        .join('')
    ...
    const profileSections = [
        {
            ...
            items: [
                { label: 'BA No', value: metadata.baNo, icon: Shield },
                { label: 'Rank', value: metadata.rank, icon: UserCircle },
                { label: 'Name', value: metadata.name, icon: UserCircle },
            ]
        },
        {
            title: 'Assignment Details',
            items: [
                { label: 'Unit Name', value: metadata.unitName, icon: Building2 },
                { label: 'Appointment', value: metadata.appointment, icon: Briefcase },
                { label: 'Formation', value: metadata.formation, icon: Flag },
            ]
        },
        {
            title: 'Contact Information',
            items: [
                { label: 'Mobile Number', value: metadata.mobile, icon: Phone },
